...
| AC-F30-02 | インデックス済みファイル「sample.md」（doc_id = "doc_sample"）をプロジェクトから削除した | `aidd-kos index` を実行する | `DELETE /documents/delete_document` が `doc_ids=["doc_sample"]` で呼び出されること（LightRAG 側の除去完了は DELETE API 成功で保証される）|
