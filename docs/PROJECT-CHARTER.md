...
`aidd-kos` を導入するだけで、AI エージェントが開発プロジェクトの「コード・設計書・業務文脈」を横断的に理解・検索できるナレッジ OS が即座に稼働する。開発者は内部ツール（LightRAG / CodeGraph 等）を意識しない。AI Agent は単一の MCP エンドポイントを通じて、常に最善の知識基盤にアクセスし、開発を最速・最大効率で推進できる。
...
| 3 | AI Agent が内部ツールを意識せず単一 MCP エンドポイントで知識にアクセスできる | ツール追加・差し替えが MCP インターフェースを変えずに完了すること（後方互換テスト PASS） |
| 4 | ナレッジエンジンをプラグイン形式で追加・入れ替えできる | 新エンジン追加が設定ファイルのみで完了し、AI Agent 側の変更が不要であること |
...
<!-- 設計の核心: aidd-kos は "Knowledge OS" であり、AI Agent に対して
     内部ツールの存在を隠蔽する抽象化レイヤーである。
     Phase 1 はシンプルな実装から始め、Phase 2 でプラグイン構造へ移行する。 -->

| 項目 | 方針 |
|------|------|
| 構成（Phase 1） | ローカル実行：単一 MCP サーバー（FastMCP）→ LightRAG REST API（localhost:9621）→ .lightrag/ |
| 構成（Phase 2〜） | 単一 MCP エンドポイント → **Knowledge Router**（エンジン選択・クエリ分配）→ 複数ナレッジエンジン（LightRAG / CodeGraph / 将来のプラグイン）|
| 抽象化原則 | AI Agent は MCP ツール名のみを知る。内部エンジンの種類・バージョン・接続先は aidd-kos が隠蔽する。エンジンの追加・差し替えは MCP インターフェースを変えない |
| プラグイン設計 | 各ナレッジエンジンは共通インターフェース（`KnowledgeEngine` プロトコル）を実装。設定ファイルで有効化・無効化可能 |
| データフロー（Phase 1） | Claude Code → MCP stdio → FastMCP server → HTTP → LightRAG API → .lightrag/ |
| データフロー（Phase 2〜） | Claude Code → MCP stdio → FastMCP server → Knowledge Router → {LightRAG, CodeGraph, ...} |
