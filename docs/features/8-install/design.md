...
- [x] specs/e2e/8-install.md（作成済み）
  → 完了条件: 全 12 AC がシナリオにトレース可能

### テスト実装（RED）

- [x] E2E テスト実装（specs/e2e/8-install.md の全 12 シナリオ）
  → 完了条件: pytest が全シナリオで失敗（RED）

### 実装

- [x] `pyproject.toml`: `typer>=0.12.0` を dependencies に追加、`aidd_kos` をパッケージに追加、エントリポイント追加
- [x] `uv sync`: 更新後に lockfile を再生成
- [x] `aidd_kos/errors.py`: エラーコード定数（ADR-001）+ `emit_error(code, remediation)` 関数
- [x] `aidd_kos/claude_settings.py`: 安全な JSON マージ実装（サイズ上限・バックアップ・ロールバック）
- [x] `aidd_kos/install.py`: `InstallOrchestrator` クラス（8 ステップ、前提チェック最初）
- [x] `aidd_kos/cli.py`: Typer app、`install` コマンド（`--project-dir` オプション付き）
- [x] `aidd_kos/__init__.py`: パッケージ初期化

### 検証

- [x] 全テストが GREEN（non-integration 42件 PASS）
- [x] `uv run ruff check . && uv run ruff format --check .` PASS
- [x] `uvx aidd-kos --help` が動作すること（エントリポイント確認）
