...
**定義**: 前回インデックス処理時の対象ファイル一覧と処理日時を記録したローカルファイル。差分検出の基準として参照される。
`.lightrag/` と同じくプロジェクト内に配置され、`.gitignore` により VCS 管理外となる。

**用例**: `aidd-kos index` 実行時にインデックス状態ファイルを参照し、前回処理後に変更されたファイルのみを抽出して再処理する。
