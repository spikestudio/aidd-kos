...
**定義**: `.claude/settings.local.json`（プロジェクトルート直下）の `mcpServers` セクションに
`cwd` フィールドなしで `aidd-kos` エントリを書き込むこと。
Epic #38 以降の `aidd-kos install` のデフォルト動作。

**用例**: プロジェクト A と B それぞれで `uvx aidd-kos install` を実行すると、
各プロジェクトに `.claude/settings.local.json` が作成され、正しい知識ベースが参照される。

---

### グローバル MCP 登録

**定義**: `~/.claude/settings.json`（ユーザーホーム）の `mcpServers` セクションに
`cwd` フィールド付きで `aidd-kos` エントリを書き込むこと。
`aidd-kos install --global` オプションで選択できる旧動作。

**用例**: `uvx aidd-kos install --global` を実行すると `~/.claude/settings.json` に
絶対パス付きのエントリが作成される（旧動作）。

---

### in-process 起動

**定義**: aidd-kos MCP サーバープロセス内部で LightRAG を Python ライブラリとして直接実行する方式。
外部ポートを占有しないため複数プロジェクトの同時起動でもポート競合が発生しない。Epic #38 で実現。

**用例**: 2 つのプロジェクトで Claude Code を同時に開いても、それぞれの `lightrag_query` が独立したインスタンスとして動作し、互いに干渉しない。

---

### MCP 設定ファイル

**定義**: aidd-kos の `mcpServers` エントリが書き込まれる設定ファイル。
プロジェクトレベルは `.claude/settings.local.json`、グローバルは `~/.claude/settings.json` の 2 種類がある。
